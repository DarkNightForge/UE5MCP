...
Last updated: 2026-06-16 (added read-only `get_actor_components` discovery tool — lists a target's components with class/creation-method/attach-parent/tags; suite verified 69/69 in-editor)  
...
- Public repo Python checks: `113 passed` in the last local verification run.
- UE automation suite is now **69/69 passing**, verified in a headless in-editor run on UE 5.7.4 (Linux source build) — including the `read_logs`, label/tag, `get_package_status`, `get_actor_properties` (read-only discovery + `ambiguous_component` refusal), `get_actor_components` (read-only component discovery), and `set_actor_property` (scalar + enum/struct-member/asset value-kind) tests.
...
| Field | Current state |
| --- | --- |
| Status | `partial` — allowlisted scalar/bool/vector/color/name/enum/asset + struct-member edits shipped; read-only `get_actor_properties` + `get_actor_components` discovery shipped |
...
| Missing | Component addressing by instance **name** (kills the `ambiguous_component` skip for multi-component actors — `get_actor_components` now surfaces the names to use), array/map/set container elements, deeper-than-one struct nesting verification, wider default allowlist. |
| Next useful slice | Component-by-name addressing on `set_actor_property`/`get_actor_properties` (optional `component_name`): now that `get_actor_components` exposes stable names, an agent can target one component among several instead of being refused `ambiguous_component`. |
...
1. **Component-by-name addressing** (`L3`) — optional `component_name` on `set_actor_property`/`get_actor_properties`: now that `get_actor_components` surfaces stable component names, target one component among several instead of being refused `ambiguous_component`.
