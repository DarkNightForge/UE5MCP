...
| Tier | Tools | Behavior |
|---|---|---|
| ReadOnly | `get_selection`, `find_actors`, `read_logs`, `get_package_status`, `get_actor_properties`, `get_actor_components`, `list_capabilities`, `preview_actions` | on `permissions.allow` → never prompt |
